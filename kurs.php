...
if(!isset($_SESSION['$emailS']) and !$_SESSION['nameS'] and !$_SESSION['statusS']){
echo '<p style="text-align: center">*Для записи и участии в данном курсе необходимо  <a href="lk.php">авторизироватся</a>.<p/>';	
}					
...
if(isset($del_kurs)){
$del_user_kurs = $pdo->query("DELETE FROM `std_1396_olymp`.`kurs_user` WHERE `kurs_user`.`id_user` = $id_users  AND `kurs_user`.`id_kurs` = $kurs_id");
if($del_user_kurs){
...
//Если есть ссылка->||
		...
		if($count_rez == 1){
			...
			echo "<p>Ссылка ещё не добавлена </p>";
		}
	}
		...
		if($members == 1){
			...
			$add_user_kurs = $pdo->query("INSERT INTO `std_1396_olymp`.`kurs_user` (`id`, `id_user`, `id_kurs`, `name-kurs`) VALUES (NULL, '$id_users', '$kurs_id', '$name_kurs');");
...
WHERE `kurs_user`.`id_kurs` =$kurs_id")->rowCount();
	...
	$res = $pdo->query("INSERT INTO `std_1396_olymp`.`link_work` (`id`, `more`, `id_kurs`, `link_file`, `id_user`) VALUES (NULL, '$more', '$kurs_id', '$path', '$id_users');");
				if($res){
...
$check_have_rez = $pdo->query("SELECT * FROM `rez_work` WHERE `id_kurs`= $kurs_id  AND `id_user` = $id_autor")->rowCount();//Проверка кол-во загруженных результатов
	...
	$resR = $pdo->query("INSERT INTO `std_1396_olymp`.`rez_work` (`id`, `comment`, `link_file`, `id_kurs`, `id_user`) VALUES (NULL, '$comment', '$path2', '$kurs_id', '$id_users');");
				if($resR){
