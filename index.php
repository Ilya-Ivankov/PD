...
<body>
	...
	$kurs = $pdo->query("SELECT `news`.`id` , `news`.`title` , `news`.`date` , `news`.`text` , `users`.`name` , `users`.`surname` 
FROM `news`
INNER JOIN `users` ON `users`.`id` = `news`.`id_autor`
INNER JOIN `kurs` ON `kurs`.`id` = `news`.`kurs` ORDER BY `news`.`date` LIMIT 0 , 3");
