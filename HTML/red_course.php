...
?>
          <div>
            <p>Название курса:</p>
            <input type="text" name="title" id="name" value="<?php echo $kurs['title'];?>">
          </div>
          <div>
            <p>Описание курса:</p>
            <textarea name="description" id="text"><?php echo $kurs['description'];?></textarea>
          </div>
          <div>
            <p>Фото для курса:</p>
	...
	$id_users = $_SESSION['idS'];
		if($_FILES['link']['name'] == ""){
	$red = $pdo->query("UPDATE `olymp`.`kurs` 
SET `title` = '$title',
...
`link` = '$path' WHERE `kurs`.`id` =$kurs_id");	
	...
	if(isset($msg)){
		echo "<div class=''>".$msg."</div> 		<script>setTimeout(function(){
