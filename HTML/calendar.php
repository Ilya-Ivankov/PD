...
<head>
    ...
    <title>Олимпиады - Календарь мероприятий</title>
</head>
<body>
    <div class="nofooter">
    ...
    <div class="main-container container">
        <div class="row">
            <div class="col-md-12">
                <h1 class="mt-3">Календарь мероприятий</h1>
				<?php
	$id_users = $_SESSION["idS"];
	$kurs = $pdo->query("SELECT `news`.`id` , `news`.`title` , `news`.`date` , `users`.`name` , `users`.`surname` 
FROM `news`
INNER JOIN `users` ON `users`.`id` = `news`.`id_autor`
INNER JOIN `kurs` ON `kurs`.`id` = `news`.`kurs`");
	$members = $pdo->query("SELECT `news`.`id` , `news`.`title` , `news`.`date` , `users`.`name` , `users`.`surname` , `kurs`.`title`
FROM `news`
INNER JOIN `users` ON `users`.`id` = `news`.`id_autor`
INNER JOIN `kurs` ON `kurs`.`id` = `news`.`kurs`")->rowCount();//кол-во записей в запросе
	if($members != 0){
		while($news = $kurs->fetch(PDO::FETCH_ASSOC)){
		echo '      <div class="card mt-3" onclick="window.location=\'news_id.php?id_news='.$news['id'].'\'" style="position: relative; cursor: pointer;">
		...
		echo '<p><a href="news_id.php?id_news='.$news['id'].'">'.$news['title'].'</a>  АВТОР: '.$news['name']." ".$news['surname'].' ДАТА ПУБЛИКАЦИИ: '.$news['date'].'</p>';
	}
	}
	else{
		echo 'Новостей нет!';
